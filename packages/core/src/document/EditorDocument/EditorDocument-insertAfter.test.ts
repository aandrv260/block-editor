...
describe("insertAfter()", () => {
  it("throws an error if we try to insert after the root", () => {
    // Arrange
    const document = new EditorDocument();
    const insertAfterRoot = () => document.insertAfter("root", SAMPLE_BLOCK2);

    // Act
    document.appendChild("root", SAMPLE_BLOCK1);

    // Assert
    assertEngineError(insertAfterRoot, {
      ExpectedErrorClass: CannotInsertAfterRootError,
      expectedCode: "DOCUMENT:CANNOT_INSERT_AFTER_ROOT",
      expectedMessage: "You cannot insert after the root!",
    });
  ...
  it("throws an error if we try to insert a block that already exists in the document", () => {
    ...
    const insertAlreadyExistingBlock = () =>
      document.insertAfter(SAMPLE_BLOCK2.id, SAMPLE_BLOCK1);
  ...
  it("throws an error if target is not a block in the tree", () => {
    ...
    const document = new EditorDocument();
    const insert = () =>
      document.insertAfter("some_target_that_does_not_exist", SAMPLE_BLOCK3);
  ...
  it("throws an error when it cannot find the target in its parent's children array based on the target's parentId property", () => {
    ...
    const insertAfter = () =>
      document.insertAfter(TOGGLE_LIST3_BLOCK.id, SAMPLE_BLOCK4);

    // Assert
    assertEngineError(insertAfter, {
  ...
  it("throws an error if the parent under which we want to insert the new block cannot have children", () => {
    // Arrange
    const document = new EditorDocument();
  ...
  it("throws an error if the parent of the target does not exist", () => {
    // Arrange
    const document = new EditorDocument();
    ...
    document.getBlock(TOGGLE_LIST2_BLOCK.id)!.parentId =
    ...
    // Assert
    assertEngineError(insertAfter, {
      ExpectedErrorClass: ParentOfTargetNotFoundError,
      expectedCode: "DOCUMENT:PARENT_OF_TARGET_NOT_FOUND",
      expectedMessage: "Parent of target does not exist!",
      expectedContext: { targetId: TOGGLE_LIST2_BLOCK.id },
    });
  });
  ...
  it("throws an error when the user tries to insert a block which has a child with ID that already exists in the document", () => {
    ...
    const tryInsert = () => {
      document.insertAfter(TOGGLE_LIST1_BLOCK.id, {
        ...TOGGLE_LIST3_BLOCK,
        children: [
          {
            ...TOGGLE_LIST4_BLOCK,
  ...
  it("throws an error if the block cannot have children and the user passes children through the payload no matter how many level deep they are in the new subtree", () => {
    ...
    const tryInsertAfter = () => {
      document.insertAfter(TOGGLE_LIST1_BLOCK.id, {
        ...
        children: [
          {
            ...TOGGLE_LIST3_BLOCK,
            children: [{ ...SAMPLE_BLOCK3, children: [SAMPLE_BLOCK4] }],
          },
        ],
      });
    };
    ...
    // Assert
    assertEngineError(tryInsertAfter, {
      ExpectedErrorClass: BlockCannotHaveChildrenError,
      expectedCode: "DOCUMENT:BLOCK_CANNOT_HAVE_CHILDREN",
      expectedMessage: `Block with ID ${SAMPLE_BLOCK3.id} cannot have children!`,
  ...
  it("copies deeply the data from the entire payload to ensure that the user won't accidentally mutate during the execution of their application in the timeline", () => {
    ...
    // Act
    document.insertAfter(TOGGLE_LIST2_BLOCK.id, SAMPLE_BLOCK2);
    ...
    expect(blockInMap).toBe(document.getRoot().children[2]);
  ...
  it("copies deeply the children in the payload to ensure that the user won't accidentally mutate during the execution of their application in the timeline", () => {
    ...
    // Act
    document.insertAfter(TOGGLE_LIST2_BLOCK.id, payload);
  ...
  it("inserts after the target correctly as direct child of the root", () => {
    // Arrange
    const document = new EditorDocument();
    ...
    document.insertAfter(SAMPLE_BLOCK1.id, SAMPLE_BLOCK4);

    // Assert
    expect(document.getRoot().children).toEqual([
      testBlockToBlock(SAMPLE_BLOCK1, "root"),
      testBlockToBlock(SAMPLE_BLOCK4, "root"),
  ...
  it("inserts after the target correctly as distant decendant of the root", () => {
    // Arrange
    const document = new EditorDocument();
    ...
    document.insertAfter(SAMPLE_BLOCK3.id, blockToInsert);

    // Assert
    const block2Children = document.getBlock(TOGGLE_LIST2_BLOCK.id)?.children;

    expect(block2Children).toEqual([
      testBlockToBlock(SAMPLE_BLOCK3, TOGGLE_LIST2_BLOCK.id),
      testBlockToBlock(blockToInsert, TOGGLE_LIST2_BLOCK.id),
      testBlockToBlock(SAMPLE_BLOCK4, TOGGLE_LIST2_BLOCK.id),
    ]);

    expect(document.getBlock(blockToInsert.id)).toEqual(
